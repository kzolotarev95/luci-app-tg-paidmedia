...
function buildYooMoneyGuideMarkup() {
	return [
		...
		'<pre class="tg-paidmedia-inline-code">grep -R "wrthub.developer.li" -n /etc/caddy\nnano /etc/caddy/Caddyfile</pre>',
		'<pre class="tg-paidmedia-inline-code">wrthub.developer.li {\n    @yoomoney path /yoomoney/*\n    reverse_proxy @yoomoney 127.0.0.1:18101\n}</pre>',
		...
		'<li>Для ЮMoney лучше указывать именно <code>https://wrthub.developer.li/yoomoney/webhook</code>.</li>',
		'</ul>',
...
function buildYooMoneyTunnelGuideMarkup() {
	return [
		...
		'<li>Если <code>curl -i http://127.0.0.1:18101/yoomoney/pay/24</code> отвечает, а <code>curl -i https://wrthub.developer.li/yoomoney/pay/24</code> даёт <code>404</code>, проблема именно в конфиге Caddy или nginx на VPS.</li>',
		'</ul>',
		'</div>',
		'</details>'
	].join('');
