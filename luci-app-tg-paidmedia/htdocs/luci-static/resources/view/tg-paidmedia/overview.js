...
function buildYooMoneyGuideMarkup() {
	return [
		...
		'<li>Публичный домен, который открывается снаружи, например <code>wrthub.developer.li</code>.</li>',
		...
		'<p>1. На VPS установите nginx:</p>',
		'<pre class="tg-paidmedia-inline-code">apt update\napt install -y nginx</pre>',
		'<p>2. На VPS добавьте конфиг nginx для домена:</p>',
		'<pre class="tg-paidmedia-inline-code">server {\n    listen 80;\n    server_name wrthub.developer.li;\n\n    location /yoomoney/ {\n        proxy_pass http://127.0.0.1:18101;\n        proxy_set_header Host $host;\n        proxy_set_header X-Forwarded-Proto https;\n        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;\n    }\n}</pre>',
		'<p>3. Проверьте и перезагрузите nginx:</p>',
		'<pre class="tg-paidmedia-inline-code">nginx -t\nsystemctl reload nginx</pre>',
		'<p>4. На роутере поднимите обратный SSH tunnel на VPS:</p>',
		...
		'<p>5. Если у вас уже есть HTTPS на VPS, используйте его. Для ЮMoney лучше указывать именно <code>https://wrthub.developer.li/yoomoney/webhook</code>.</p>',
		...
		'<li>Если в логах пусто, значит URL не сохранился или VPS/tunnel/nginx не пропускает запрос.</li>',
		'</ul>',
		...
		'<li>Бот пришлет ссылку вида <code>https://wrthub.developer.li/yoomoney/pay/6</code>.</li>',
		...
		'<ul class="tg-paidmedia-info-list">',
		'<li>Нет <code>POST /yoomoney/webhook</code>: проблема с URL, VPS, nginx или tunnel.</li>',
...
return view.extend({
	...
	renderForm: function() {
		...
		o = s.option(form.Value, 'home_image', '\u0428\u0430\u043f\u043a\u0430 \u0431\u043e\u0442\u0430 (URL \u0438\u043b\u0438 file_id)');
		o.rmempty = true;
		o.placeholder = 'https://example.com/header.jpg';
		o.description = '\u0411\u043e\u0442 \u043f\u043e\u043a\u0430\u0436\u0435\u0442 \u044d\u0442\u043e \u0444\u043e\u0442\u043e \u043d\u0430\u0434 \u0433\u043b\u0430\u0432\u043d\u044b\u043c \u044d\u043a\u0440\u0430\u043d\u043e\u043c. \u041c\u043e\u0436\u043d\u043e \u0432\u0441\u0442\u0430\u0432\u0438\u0442\u044c \u043f\u0440\u044f\u043c\u043e\u0439 URL \u043a\u0430\u0440\u0442\u0438\u043d\u043a\u0438 \u0438\u043b\u0438 Telegram file_id.';
